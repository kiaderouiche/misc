...
\end{solution}
 ...
 \chapter{Graphiques}
La visualisation de fonctions d'une ou deux variables, d'une série de données, facilite la perception de phénomènes mathématiques ou physiques et permet de conjecturer des résul Équations sans solution explicitement efficacement. Dans ce chapitre, on illustre sur des exemples les capacités graphiques de SymPy.
...
ces quatre cas, puis donnons quelques exemples de tracés de données.
 ...
 Les tracés de courbes paramétrées $\left( x = f\left(t\right), y=f\left(t\right)\right)$ sont réalisés par la commande Curve\footnote{du module sympy.geometry(quand on le verra dans le chapitre)}$\left(\left(f\left( t\right)\right), \left(\left(g\left( t\right)\right), \left(t, a, b\right)\right)$ ou $\left[a, b\right]$ est l'intervalle parcouru par le paramètre.
 \\
