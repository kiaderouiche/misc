...
Considérons l'inéquation suivante $ln\left(x-2 \right) + ln\left(x+1 \right) \geqslant ln\left(4\right)$
La bibliothèque SymPy implémente des modules pour calculer des inéquations à une et plusieurs inconnus qui fait appel à la classe qui l'objet..., plus simplement on montre dans l'exemple suivant
 \begin{python}
In [153]: solve(x**(1/x)==(1/x)**x, x)
Out[153]: False
\end{python}
\subsection{L’inéquation trigonométrique}
