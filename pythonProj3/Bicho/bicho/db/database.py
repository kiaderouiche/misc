...
class DBDatabase:
    ...
    def insert_issue(self, issue, tracker_id):
        ...
        try:
            ...
            db_issue.type = str(issue.type)
            db_issue.summary = str(issue.summary)
            db_issue.description = str(issue.description)
            db_issue.status = str(issue.status)
            db_issue.resolution = str(issue.resolution)
            db_issue.priority = str(issue.priority)
    ...
    def _get_db_supported_tracker(self, name, version):
        ...
        db_sup = self.store.find(DBSupportedTracker,
                                 DBSupportedTracker.name == str(name),
                                 DBSupportedTracker.version == str(version)).one()
        if not db_sup:
    ...
    def _get_db_people(self, user_id):
        ...
        try:
            db_people = self.store.find(DBPeople,
                                        DBPeople.user_id == str(user_id)).one()
    ...
    def _get_db_comment(self,comment, issue_id, tracker_id):
        ...
        db_comment = self.store.find(DBComment,
                                     ...
                                     DBComment.text == str(comment.comment),
