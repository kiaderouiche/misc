...
class DBManiphestBackend(DBBackend):
    ...
    def insert_project(self, store, project):
        try:
            db_project = DBManiphestProject(str(project.name),
                                            str(project.phid))
    ...
    def _get_db_project(self, store, phid):
        db_project = store.find(DBManiphestProject,
                                DBManiphestProject.phid == str(phid)).one()
        if not db_project:
            raise NotFoundError(f'Maniphest project {phid} not found')
        return db_project

    def _get_db_issues_projects(self, store, issue_id) -> str:
...
class Conduit(object):
    ...
    def projects(self, phids):
        ...
        prjs = [p for p in list(result['data'].values())]

        return prjs

    def tasks(self, offset=None, limit=None, as_id=False)->str:
        ...
        if not as_id:
            ...
            start = offset
            stop = start + limit
            params['ids'] = list(range(start, stop))
        ...
        ph_tasks = sorted(list(result.items()),
...
class Maniphest(Backend):
    ...
    def check_auth(self):
        ...
        try:
            ...
            return True
        except (requests.exceptions.HTTPError, ConduitError) as e:
            printerr("Error: %s" % e)
            return False

    def get_issue_from_task(self, pht)->str:
    ...
    def get_events_from_transactions(self, phtrans)-> (str, str):
        comments = []
        changes = []
    ...
    def get_identity(self, ph_id)->str:
        if not ph_id:
            return None
    ...
    def get_identities(self, phids)->str:
        ids = []
        request = []
    ...
    def get_projects(self, phids)->str:
        prjs = []
        request = []
    ...
    def fetch_and_store_tasks(self):
        ...
        if self.from_id:
            ...
            printout(f"Fetching tasks from {count} id to {count + self.max_issues - 1} id")
        else:
            as_id = False

            if last_mod_date:
                printdbg(f"Last modification date stored: {last_mod_date}")
            if self.start_from:
                printdbg(f"Ignoring tasks after {str(self.start_from)}")

            printout(f"Fetching tasks from {count} to {count + self.max_issues}")
    ...
    def run(self):
        printout(f"Running Bicho - {self.url}")
