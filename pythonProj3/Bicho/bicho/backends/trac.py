...
class TracRPC(object):
    ...
    def changes(self, ticket_id) -> str:
...
class Trac(Backend):
    ...
    def insert_tracker(self, url) -> str:
    ...
    def get_datetime_from_json_obj(self, obj) -> str:
        dt = obj['__jsonclass__'][1]
        return dateutil.parser.parse(dt)

    def get_uri(self, ticket_id) -> str:
    ...
    def get_issue_from_ticket(self, ticket) -> str:
        printdbg(f'Parsing ticket {ticket[0]}')
        ...
        printdbg(f"Ticket {ticket[0]} parsed")
    ...
    def get_events_from_changes(self, ticket_changes):
        ...
        for ch in ticket_changes:
            ...
            field = ch[2]
            old_value = str(ch[3]) if ch[3] else None
            new_value = str(ch[4]) if ch[4] else None
    ...
    def fetch_and_store_tickets(self):
        ...
        for ticket_id in trac_tickets:
            printdbg(f"Fetching ticket {str(ticket_id)}")
        ...
        printout(f"Done. {nbugs} bugs analyzed from {len(trac_tickets}")

    def run(self):
        printout(f"Running Bicho with delay of {self.delay} seconds - {self.url}")

        try:
            self.fetch_and_store_tickets()
        except (requests.exceptions.HTTPError, TracRPCError) as e:
            printerr("Error: {e}")
            sys.exit(1)
