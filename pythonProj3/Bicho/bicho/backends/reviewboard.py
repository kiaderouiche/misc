...
from bicho.common import Tracker, People, Issue, Change
...
CODE_REV_FIELD = str('Code-Review')

NEW_STATUS = str('NEW')
UPLOADED_STATUS = str('UPLOADED')
MERGED_STATUS = str('MERGED')
ABANDONED_STATUS = str('ABANDONED')
...
class ReviewBoardAPIClient(object):
    ...
    def review_requests(self, offset=None, limit=None, group=None, last_date=None) -> str:
    ...
    def review_request_reviews(self, review_req_id) -> str:
    ...
    def review_request_changes(self, review_req_id) -> str:
    ...
    def call(self, method, params) -> str:
...
class ReviewBoard(Backend):
    ...
    def get_review_request(self, raw_rq):
        ...
        # Get changes
        printdbg(f'Fetching review request reviews from {rq_id}')
    ...
    def fetch_review_changes(self, rq):
        ...
        for raw_ch in reversed(raw_changes):
            ...
            if 'diff' in ch_fields:
                revision = str(ch_fields['diff']['added']['revision'])
    ...
    def fetch_and_store(self):
        ...
        printout(f'Fetching reviews requests from {offset} to {offset + self.max_issues}')
    ...
    def run(self):
        printout(f'Running Bicho - url: {self.url}')

        try:
            self.fetch_and_store()
        except (requests.exceptions.HTTPError, ReviewBoardAPIError) as e:
            printerr(f'Error: {e}')
            sys.exit(1)
