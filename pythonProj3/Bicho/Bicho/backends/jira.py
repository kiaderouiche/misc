...
from bs4 import Comment as BFComment
...
class DBJiraBackend(DBBackend):
    ...
    def __return_unicode(self, str) -> str:
        '''
    ...
    def parse_changes(self):
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if len(cols) == 3:
                    field = str(cols[0].contents[0].strip())
                    if field == "Assignee":
                        old = str(self._get_identifier(cols[1]))
                        new = str(self._get_identifier(cols[2]))
                    else:
                        old = str(cols[1].contents[0].strip())
                        new = str(cols[2].contents[0].strip())
...
class JiraConnection(object):
    ...
    def login(self, url, user=None, password=None):
        ...
        cookie_h = urllib.request.HTTPCookieProcessor(cookie_j)
        ...
        opener = urllib.request.build_opener(cookie_h)
        ...
        printout(f"Logged in Jira as {user}")
        printdbg(f"Jira session cookies: {self.cookies}")
    ...
    def urlopen_auth(self, url):
        ...
        if self.is_auth_session():
            q = [str(c) + '=' + v for c, v in list(self.cookies.items())]
    ...
    def safe_xml_parse(self, url_issues, handler):
        ...
        try:
            ...
            try:
                ...
                printerr(f'Error parsing URL: {url_issues}')
                raise
        f.close()
    ...
    def run(self):
        ...
        if (project.split("-").__len__() > 1):
            ...
            if self.last_mod_date:
                ...
                printdbg(f'Last bugs cached were modified at: {self.last_mod_date}')
            ...
            printout(f'Done. {bugs_number} bugs analyzed')
