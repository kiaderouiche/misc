...
class DBAlluraBackend(DBBackend):
    ...
    def insert_issue_ext(self, store, issue, issue_id):
        ...
        try:
            ...
            db_issue_ext.labels = str(issue.labels)
            db_issue_ext.private = bool(issue.private)
            db_issue_ext.ticket_num = int(issue.ticket_num)
            db_issue_ext.discussion_thread_url = str(issue.discussion_thread_url)
            db_issue_ext.related_artifacts = str(issue.related_artifacts)
            db_issue_ext.custom_fields = str(issue.custom_fields)
...
class Allura():
    ...
    def parse_changes(self, activity):
        ...
        for entry in activity['entries']:
            ...
            while description:
                ...
                change = Change(str(field), str(old_value), str(new_value), by, update)
    ...
    def run(self):
        ...
        if last_mod_date:
            ...
            printdbg("Last bugs analyzed were modified on: {last_mod_date}")
        ...
        self.url_issues += urllib.parse.quote("mod_date_dt:[" + time_window + "]")
        ...
        while start_page <= total_pages:
            ...
            for bug in bugs:
                try:
                    ...
                    printerr(f"UnicodeEncodeError: the issue {issue_data.issue} couldn't be stored")
            start_page += 1
