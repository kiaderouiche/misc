...
from bicho.db.database import DBBackend, DBTracker, DBIssue, get_database, NotFoundError
...
class LPBackend(Backend):
    ...
    def run(self):
        ...
        printdbg(f'{len(projects)} projects to analyze')
        ...
        for p in projects:
            ...
            if nbugs == 0:
                printout(f'No bugs found on {p.name}')
                continue
            else:
                printout(f'{nbugs} bugs found on {p.name}')
