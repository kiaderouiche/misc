...
class DBSourceForgeBackend(DBBackend):
    ...
    def insert_issue_ext(self, store, issue, issue_id):
        ...
        try:
            ...
            db_issue_ext.category = str(issue.category)
            db_issue_ext.group = str(issue.group)
...
class SourceForgeParser():
    ...
    def parse_issues_list(self, html):
        ...
        ids = []
        for href in hrefs:
            query = urllib.parse.urlsplit(href.get('href')).query
            ids.append(urllib.parse.parse_qs(query)['aid'][0])
        return ids

    def parse_issue(self, html) -> str:
        '''
        '''
        soup = bs4.bs4(html,
                                           convertEntities=bs4.bs4.XHTML_ENTITIES)
    ...
    def get_total_issues(self, html)-> int:
        '''
        '''
        soup = bs4.bs4(html)
    ...
    def __parse_issue_id(self, soup):
        ...
        '''
        try:
            m = ISSUE_ID_PATTERN.match(str(soup.title.string))
            return m.group(1)
        except:
    ...
    def __parse_issue_summary(self, soup):
        ...
        '''
        try:
            m = ISSUE_SUMMARY_PATTERN.match(str(soup.title.string))
            return m.group(1)
        except:
            printerr('Error parsing issue summary')
            raise SourceForgeParserError('Error parsing issue summary')

    def __parse_issue_description(self, soup) -> str:
        '''
        '''
        try:
    ...
    def __parse_issue_comments(self, soup) -> str:
        '''
        '''
        comments = []
        try:
            ...
            for art in artifacts:
                ...
                comment['desc'] = self.__clean_str(''.join(rawdesc.contents))
    ...
    def __parse_issue_attachments(self, soup):
        ...
        try:
            ...
            for f in files:
                ...
                aux = f.findAll('td')
                attch['filename'] = self.__clean_str(''.join(aux[0].contents))
                attch['desc'] = self.__clean_str(''.join(aux[1].contents))
...
class SourceForge():
    ...
    def __check_tracker_url(self, url):
        ...
        if query is not None:
            ...
            qs = list(urllib.parse.parse_qs(query).keys())
