...
class Redmine():
    ...
    def _get_author_identity(self, author_id):
        ...
        identity = None
        try:
            f = urllib.request.urlopen(author_url)
            person = json.loads(f.read())
            identity = person['user']['mail']
        except (urllib.error.HTTPError, KeyError):
            printdbg(f"User with id {author_id} has no account information")
            identity = author_id

        self.identities[author_id] = identity
        return identity

    def analyze_bug(self, issue_redmine) -> str:
        #print(issue_redmine)
        ...
        print(f'Issue #{issue_redmine["id"]} updated on {issue.updated_on}')

        return issue

    def _get_issues_url(self, updated_on=None) -> str:
    ...
    def _get_issue_url(self, issue_id) -> str:
        issue_url = self._get_redmine_root(Config.url)
        issue_url = issue_url + "issues/" + str(issue_id) + ".json?include=journals"
        return issue_url
    ...
    def _parse_journals(self, issue, issue_id):
        ...
        f = urllib.request.urlopen(issue_url)
        ...
        for journal in journals:
            ...
            for detail in journal["details"]:
                field = detail["name"]
                old_value = str(detail.get("old_value", str(None)))
                new_value = str(detail.get("new_value", str(None)))

                # Change status value
                if field == "status_id":
                    field = str("status")
                    old_value = self.statuses.get(old_value, str(None))
                    new_value = self.statuses.get(new_value, str(None))

                change = Change(field, old_value, new_value, people, dt)
                issue.add_change(change)

    def _parse_html_change(self, html) -> str:
    ...
    def remove_unicode(self, str) ->str:
        '''
    ...
    def run(self):
        '''
        '''
        printout(f"Running Bicho with delay of {str(self.delay} seconds")
