...
class SoupHtmlParser():
    ...
    def __init__(self, html, idBug):
        self.html = html
        self.idBug = idBug
        self.field_map = {'Status': 'status', 'Resolution': 'resolution'}
    ...
    def parse_changes(self):
        ...
        for row in rows[1:]:
            ...
            if len(cols) == 5:
                ...
                person_email = str(person_email.replace('&#64;', '@'))
...
class BGBackend(Backend):
    ...
    def _healthy_url(self, url):
        ...
        if len(component) > 1:
            url = tokens[0] + 'product=' + urllib.parse.quote(component[0])
            url += "&component=" + urllib.parse.quote(component[1])
        else:
            url = tokens[0] + 'product=' + urllib.parse.quote(tokens[1])
        return url

    def _urlopen_auth(self, url)-> str:
        '''
        ...
        while keep_trying:
            ...
            keep_trying = False
            try:
                aux = urllib.request.urlopen(url)
    ...
    def _is_issue_url(self, url) -> str:
        '''
    ...
    def _get_domain(self, url) -> str:
        ...
        domain = urllib.parse.urljoin(result.scheme + '://' + result.netloc + '/',
                                  newpath)
        return domain

    def _get_login_url(self, base_url) -> str:
        pos = base_url.rfind('buglist')
        url = base_url[:pos] + 'index.cgi'
        return url

    def _get_info_url(self, base_url) ->str:
