...
class ParseMessage:
        ...
        for header in self.common_headers:
            ...
            if msg:
                try:
                    ...
                    print('TypeError: msg: %s % msg', file=sys.stderr)
    ...
    def __decode(self, s, charset='latin-1', sep=' '):
        ...
        try:
            ...
            print('WARNING: charset: %s' % charset, end=' ', file=sys.stderr)
...
class MailArchiveAnalyzer:
    ...
    def get_messages(self):
        ...
        for message in mbox:
            ...
            if not filtered_message['message-id']:
                ...
                print('=> message-id not present for:', file=sys.stderr)
    ...
    def make_msgid(self, from_addr, message):
        try:
            ...
            domain = 'mlstats.localdomain'

        m = hashlib.md5(message.encode('utf-8')).hexdigest()
        return '<%s.mlstats@%s>' % (m, domain)
