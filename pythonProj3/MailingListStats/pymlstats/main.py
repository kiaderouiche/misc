...
class Application(object):
    def __init__(self, driver, user, password, dbname, host,
        ...
        self.__print_output(f"{total_messages} messages analyzed" )
        self.__print_output("%d messages stored in database %s" %
                            (stored_messages, dbname))
        self.__print_output("f{non_parsed} messages ignored by the parser")
    ...
    def __get_backend(self, mailing_list) -> str:
        def guess_backend(ml) -> str:
    ...
    def __set_archives_to_analyze(self, mailing_list, archives) -> str:
        # today = datetime.datetime.today().strftime(datetimefmt)
        today = datetime.datetime.today()
    ...
    def __analyze_list_of_files(self, mailing_list, archives_to_analyze):
        ...
        for archive in archives_to_analyze:
            self.__print_output(f'Analyzing {archive.filepath}')
            ...
            try:
                ...
                self.__print_output(f"Invalid file: {archive.filepath} - {str(e)}. Skipping.")
                continue
    ...
    def __get_gmane_total_count(self, mailing_list_url, download_url) ->str:
